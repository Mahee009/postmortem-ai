...
if __name__ == "__main__":
    ...
    test_profile = {
        ...
        "biggest_worry": "Not converting free users to paid",
        "query_text": "B2B SaaS early-revenue freelancer time tracking invoicing tool struggling to convert free users to paying customers bootstrapped small team"
    }

    results = asyncio.run(retrieve_similar_failures(test_profile, top_k=5))
    print(f"\nFound {len(results)} similar failures:")
    for r in results:
        print(f"  - {r.get('startup_name', '?')}: {r.get('raw_summary', '')[:80]}...")
