...
if __name__ == "__main__":
    ...
    results = search_similar(
        "B2B SaaS tool for small businesses, 6 months in, no paying customers",
        top_k=3,
        client=client,
    )
    for r in results:
        print(f"\n[{r['similarity_score']:.2f}] {r.get('startup_name', 'Unknown')} — {r.get('primary_failure_cause', '?')}")
        print(f"  {r.get('raw_summary', '')[:100]}...")
