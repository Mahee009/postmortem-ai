...
from agents.competitors import find_competitors
...
async def analyze(req: AnalyzeRequest):
    ...
    try:
        ...
        # Competitor lookup — non-fatal if Serper key is missing or call fails
        competitors: list[dict] = []
        try:
            competitors = await find_competitors(result.get("startup_profile", {}))
        except Exception as e:
            logger.warning(f"Competitor lookup failed (non-fatal): {e}")

        return AnalyzeResponse(thread_id=thread_id, competitors=competitors, **result)
    except Exception as e:
