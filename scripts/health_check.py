...
def check_qdrant():
    try:
        ...
        if "your-cluster" in url:
            return False, "QDRANT_URL is still the placeholder — update it in .env"
...
def check_env():
    required = ["OPENROUTER_API_KEY", "FIRECRAWL_API_KEY"]
...
async def main():
    ...
    checks = [
        ...
        ("OpenRouter LLM", await check_openrouter()),
        ("Qdrant",        check_qdrant()),
    ...
    if all_ok:
        ...
        console.print("  OpenRouter: https://openrouter.ai  (free, no credit card)")
        console.print("  Qdrant:     https://cloud.qdrant.io  (free tier)")
        console.print("              or: docker run -p 6333:6333 qdrant/qdrant")
        console.print("  Firecrawl:  https://firecrawl.dev  (500 free credits)\n")
